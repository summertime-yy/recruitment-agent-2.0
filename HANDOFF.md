...
- **状态**：CLOSED · 未自行 FF-merge（按用户指示留待指挥官处置）· anchor `976258c`（commit 5 · 终态 FAILED + executions 补列）· 6 commit（`925838d` red-test skeleton + test_factories 隔离修复 → `1452e8e` session_factory 透传（PR-21 §3.5 漏网）+ §五 边界 1 归因 → `b9738ec` input_schema 注入 + HYDRATION_HINTS → `b59f8a3` candidate_ids 单/多元素兼容 → `976258c` 终态 FAILED + executions.input_params/error_message 补列 → 报告 commit）· PR-28 分支保留（`feat/pr-28-fix-execute-db-passthrough-and-schema-injection`）
...
- **修复面（纯后端）**：① `engine.run_execute` 增 `session_factory` kwarg 并透传给 `_background_execute` + `app.api.v1.agent.execute-plan` 端点 L177 传 `session_factory=async_session_factory`；② `engine._format_dispatchable_tools` 重写，每个 builtin/skill 追加 `input_schema` JSON 行 + HYDRATION_HINTS 中文提示行（`hydraion.py` 引入 `HYDRATION_HINTS` 常量，key 与 `HYDRATION_RULES` 严格一致）；③ `_hydrate_candidate_profile` 兼容 `candidate_ids: ["res_xxx"]` 单元素（log warning 后当 `candidate_id` 用），多元素抛 `ToolParamError` 指向 `candidate-merge`；④ `_make_db_execution_writer.writer` 接受 `tool_input=` 写入 `executions.input_params`（JSONB），FAILED 终态存 `executions.error_message`（Text）；`act.run_act.on_step_start` 用 `inspect.signature` 检测 callback 是否接受 `tool_input` 自动分流 2-arg / 3-arg（PR-20 SSE collector 兼容）；`factories.py` 默认 `skill_id` 加 uuid 后缀（隔离 dev DB 中已 seed 的 `jd-candidate-matching` 行）+ 新增 `build_task` 工厂。**零 frontend / schema / migration 变更**（`executions.input_params` / `error_message` 列已存在）。
...
- **测试**：9 个 PR-28 测试（`test_stage5_pr28_execute_injection.py`）— TC-PR28-1 `run_execute_endpoint_passes_session_factory`（AST 断言）· TC-PR28-2 `format_dispatchable_tools_injects_input_schema_and_hints`（markdown 渲染）· TC-PR28-3 `hydrate_does_not_silently_succeed_when_db_none`（dispatch db 真值）· TC-PR28-4 `hydration_receives_real_db_and_pulls_parsed_content`（§五 边界 1 归因）· TC-PR28-5/6 `hydration_accepts_single_element_candidate_ids` / `rejects_multi_element_candidate_ids`（plural 兼容 + 多元素硬错）· TC-PR28-7 `execution_writer_persists_input_params_and_error`（writer 字段）· TC-PR28-8 `background_execute_ends_task_failed_on_skill_exception`（Task 终态 FAILED）；外加 `test_factories.py` 修复 `test_factories_build_skill_execution_log`（隔离基线 1 failed → 转绿）
- **本地门禁**：backend `pytest` **153 passed / 2 skipped**（基线 144 + 9 PR-28 净增，超过 §七 期望 150）· `ruff check app/ tests/` **All checks passed**（PR-28 改动文件全 clean；`ruff check .` 仅 untracked UAT 工具 `_diag_uat.py` / `_seed_resume2_uat.py` 报错，非 PR 范围）· frontend **零改动**（`git diff --stat master -- frontend/` 为空）· §七 第四道门 "等效 curl e2e"：TC-PR28-4 走真 db 路径拉 parsed_content + existing_tags（= §4.1 §2.2 修复的实测证据）+ TC-PR28-7/8（executions 审计列 + Task 终态 FAILED）。
