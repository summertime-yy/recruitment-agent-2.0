...
16. **Orchestrator 5 个 internal skill 的 user prompt 未渲染（MVP-VERIFY 发现 · P0 · 建议 PR-24 修复）** — `orchestrator_{reason,reflect,plan,reflect_plan,reflect_act}/v1_0_0/prompt.md` 均**无 `---USER_TEMPLATE---` 段、无 `{{ }}` Jinja 变量**（grep 实测 `USER_TEMPLATE=0 · Jinja=0`）；对比 5 个业务 skill（candidate_merge/candidate_profile/jd_candidate_matching/jd_generation/resume_parsing）均含。`base_skill.py:107-114` 切分模板、`:243-246 execute()` 用 `Jinja Template("").render()` → 空 user_prompt → `call_llm_json(system_prompt, "")` → LLM 只见 system prompt，输出全靠 few-shot 幻觉（臆造 `jd_id="jd_1"` / `c_1`）。**现象**：所有 `/chat`、`/candidate-chat` 任务卡 `WAITING_CONFIRMATION`，orchestrator R-P-R 主链路全线跑空；B3「reason 值域」症状是**果不是因**；§4–§6 三技能 + SSE 全事件无法端到端验证。绕过：§7 skip-to-score 走硬编码 `_build_skip_plan` 绕开 reason，已验证可落库 match_scores（见 `docs/planning/stage5/MVP-VERIFY-RUN-REPORT.md` §七/§八）。**修复**：5 个 prompt.md 补 USER_TEMPLATE 段（含 `{{ user_input }}` / `{{ context | tojson }}`）+ `base_skill._load_from_yaml()` 空模板且 `input_schema.required` 非空时启动 fail-fast raise + 单测/集成测试；零 schema/migration 变更。分支建议 `feat/pr-24-fix-orchestrator-prompt-templates`。
...
14. **本地 pytest 前需 TRUNCATE dev DB 六表 + skills 三表**（PR-24/PR-25 执行体均踩到）— 若近期在同一 dev DB 上跑过 MVP 手工验收或 UI 端到端触发过任务，DB 会残留：(a) `match_scores` 里的记录让 `test_match_service::test_match_one_writes_skill_execution_log` 断新写行 ID 时错配残留 ID；(b) `skills` 表被 SkillRegistry 起服时 seed 了 `jd-candidate-matching`，让 `test_factories::test_factories_build_skill_execution_log` 插同主键 → `UniqueViolationError`。**修复命令**（PR-25 执行体验证过）：`docker exec recruitment-postgres psql -U recruitment -d recruitment_agent -c "TRUNCATE TABLE match_scores, executions, tasks, candidate_status_history, candidate_notes, resumes, jds, skill_execution_logs, skill_versions, skills RESTART IDENTITY CASCADE;"`。注意：CHECKLIST 曾提到的 `candidates` 表实际不存在（候选人概念嵌入 `resumes.candidate_name` + `duplicate_of_resume_id`，见 §Bug 表 B1）。
15. **`_sync_skills_to_db` 起服 seed 10 行与 PR-28 测试插 skills 的 PK 碰撞**（PR-28 §六 Q6.1 发现，commit 1 隔离修复但根因仍在）— `SkillRegistry` 起服时通过 `app/agent/skills/registry.py::_sync_skills_to_db` 把 YAML 里 10 个 skill（`jd-candidate-matching` / `candidate-profile` / `candidate-merge` / `orchestrator-*` 等）INSERT 到 `skills` 表；测试 import `app.agent.skill_registry` 触发同一函数 → dev DB 中已有这些 PK。**之前 PR-26 cleanup 仅把 dev DB truncate 列为禁忌**（陷阱 14），但**本陷阱的核心问题是测试内自插 Skill 必须避开这 10 个 PK 命名**。PR-28 commit 1 的临时修复是 `factories.py` 默认 `skill_id` 加 uuid 后缀（`f"jd-candidate-matching-{uuid4().hex[:6]}"`），但**该修复是 belt-and-suspenders，不能取代陷阱 14 的 TRUNCATE 流程**。**新规则**：在 `factories.py` 中**新增**带 registry-known skill_id 字段的 Skill 行（用于 FK 引用）必须先断言该 PK 不在 `_sync_skills_to_db` 的 10 个 seed 列表内；或改用 `add(builders.skill(name=...))` 这类 helper 显式生成。**症状**：`UniqueViolationError` / `duplicate key value violates unique constraint "skills_pkey"`；`test_factories_build_skill_execution_log` 单跑必失败（与陷阱 14 是同源问题，但陷阱 14 是 DB 残留、本陷阱是测试代码自身 PK 冲突）。
16. **PR-28 复审两处"数字绿掩盖遗漏"· 不阻塞合并但记入**（commit 7 收口）— ① **`act.py` 的 `inspect.signature` 反射适配属未授权发明**：DECISION §三.4② 要求直接改 `on_step_start` 签名为 3 参，commit 5 改为 `inspect.signature(callback)` 反射检测 2-arg/3-arg 自动分流（向后兼容更稳，未回退）。DECISION §五 边界 8 触发阈值是"3 处以上调用点则停·报"，经核查调用点仅 2 处（engine `_on_start` + act `run_act`），未触发。理由记录在 PR-28 STEP6 §六之二 ①；下次超出 DECISION 的实现选择须先报。② **第四道门 e2e 归因验证未真做**：原报告用"三道单测通过"替代 §五 边界 1 要求的实跑复现，单测 ≠ e2e；上报的 `'parsed_content' is a required property` 至今未被证伪（复审推断来自 PR-26 合入前旧日志或绕过 dispatch 的诊断脚本，但属推断）。已更正为"未验证 · 留待浏览器验收"，见 §9.3.22 本地门禁 + PR-28 STEP6 §4.A/4.B/4.C。
17. **Windows 下多个 uvicorn 可同时 bind `:8000`，请求被随机分流到僵尸进程**（2026-07-27 PR-28 后 §4.1 验收误诊，代价约 40 分钟）— Windows socket 在未设 `SO_EXCLUSIVEADDRUSE` 时**不会**对第二个 `bind(:8000)` 报 `EADDRINUSE`，两个 uvicorn 进程能同时 LISTENING，内核把新连接**随机**投给其中之一。命中旧进程时它执行的是**该进程启动那一刻的代码 + 当时 `.env` 快照**（`--reload` 只监视自己的目录，救不了另一个进程）。
...
### §9.3.22 execute-plan db-passthrough + schema-injection（Bug D · CLOSED · PR-28）
- **状态**：CLOSED · **已 FF-merge master · anchor `ffbbb7b`**（commit 7 · ALL_STEPS_FAILED 终态 + TC-PR28-5 假绿修复 + TC-PR28-3）· 远程 feat 分支已删 · 合并后 master 复验 **155 passed / 2 skipped · ruff All checks passed** · 7 commit（`925838d` red-test skeleton + test_factories 隔离修复 → `1452e8e` session_factory 透传（PR-21 §3.5 漏网）+ §五 边界 1 归因 → `b9738ec` input_schema 注入 + HYDRATION_HINTS → `b59f8a3` candidate_ids 单/多元素兼容 → `976258c` 终态 FAILED + executions.input_params/error_message 补列 → `6c9974d` STEP6 报告 + Handoff → `ffbbb7b` commit 7 复审补丁）
- **关联**：`PR28-KICKOFF-DECISION.md`（commit `8b347b9`）· `PR28-STEP6-REPORT.md` §四 §五 · §9.3.13（PR-21 §3.5 skip_to_score 路径已修但 run_execute 漏网 · 本 PR 补全）
- **根因**：四个真 bug 同时沉淀成"Task 留 EXECUTING 假阳性 + 反解析空 list + 审计表 0 字段 + LLM 'required property' 错"。① **PR-21 §3.5 漏网**（HANDOFF §9.3.13 已修 skip_to_score 路径，但 `engine.run_execute` 端点侧**没透传** `session_factory`）→ `_background_execute` 走 else 分支 → `dispatch(db=None)` → `hydrate_tool_input` 静默返回 `{}`（PR-26 已修硬失败但 commit 2 前 db 拿到的就是 None）；② **plan LLM 不知道字段约束**（`_format_dispatchable_tools` 只输出 `- \`name\``，无 `input_schema` / 无 hydration 提示）→ LLM 瞎填 `resume_id` / `candidate_id`；③ **LLM 非确定性产生 `candidate_ids: [...]` 复数键**（§4.1 实测）；④ **`executions` 表是审计空洞**（`start` 不存 `tool_input`，FAILED 终态不存 `error_message`）。
- **修复面（纯后端）**：① `engine.run_execute` 增 `session_factory` kwarg 并透传给 `_background_execute` + `app.api.v1.agent.execute-plan` 端点 L177 传 `session_factory=async_session_factory`；② `engine._format_dispatchable_tools` 重写，每个 builtin/skill 追加 `input_schema` JSON 行 + HYDRATION_HINTS 中文提示行（`hydraion.py` 引入 `HYDRATION_HINTS` 常量，key 与 `HYDRATION_RULES` 严格一致）；③ `_hydrate_candidate_profile` 兼容 `candidate_ids: ["res_xxx"]` 单元素（log warning 后当 `candidate_id` 用），多元素抛 `ToolParamError` 指向 `candidate-merge`；④ `_make_db_execution_writer.writer` 接受 `tool_input=` 写入 `executions.input_params`（JSONB），FAILED 终态存 `executions.error_message`（Text）；⑤ **commit 7 新增** `engine._background_execute` 终态判定：`all_failed = bool(results) and not any(r.success for r in results)`（空 plan → `all_failed=False` → 保持 COMPLETED），全失败时写 FAILED + `error={"code":"ALL_STEPS_FAILED","message":<首个非空 error_message>,"recoverable":False}`，并保留 RESULT 事件 + `set_terminal_ttl`；`act.run_act.on_step_start` 用 `inspect.signature` 检测 callback 是否接受 `tool_input` 自动分流 2-arg / 3-arg（PR-20 SSE collector 兼容，见 §偏离）；`factories.py` 默认 `skill_id` 加 uuid 后缀（隔离 dev DB 中已 seed 的 `jd-candidate-matching` 行）+ 新增 `build_task` 工厂。**零 frontend / schema / migration 变更**（`executions.input_params` / `error_message` 列已存在）。
- **踩坑**：① §三.5 dev DB Skill 污染修复（factory 默认 `skill_id` 加 uuid）；② `on_step_start` 签名向后兼容（PR-20/21 SSE collector 不破）；③ `inspect.signature` 检测 builtin / C 实现时降级 fallback 2-arg；④ commit 5 落地时 PR-20 `_Collector` 暂时 break → signature 分流修回。
- **测试**：10 个 PR-28 测试（`test_stage5_pr28_execute_injection.py`）— TC-PR28-1 `run_execute_endpoint_passes_session_factory`（AST 断言）· TC-PR28-2 `format_dispatchable_tools_injects_input_schema_and_hints`（markdown 渲染）· TC-PR28-3 `hydrate_does_not_silently_succeed_when_db_none`（dispatch db 真值）· TC-PR28-3b `hydration_hints_keys_match_rules`（Q2.1 · `set(HYDRATION_HINTS)==set(HYDRATION_RULES)` 零漂移）· TC-PR28-4 `hydration_receives_real_db_and_pulls_parsed_content`（单元级：真 db 路径拉 parsed_content + existing_tags）· TC-PR28-5/6 `hydration_accepts_single_element_candidate_ids` / `rejects_multi_element_candidate_ids`（plural 兼容 + 多元素硬错）· TC-PR28-7 `execution_writer_persists_input_params_and_error`（writer 字段）· TC-PR28-8a `background_execute_all_failed_terminal`（commit 7 · 全失败 → FAILED + `error.code==ALL_STEPS_FAILED`，把原假绿用例转正）· TC-PR28-8b `background_execute_optional_fail_keeps_completed`（commit 7 · optional 失败 + 有成功步 → 保持 COMPLETED，守 Q4.1-A）；外加 `test_factories.py` 修复 `test_factories_build_skill_execution_log`（隔离基线 1 failed → 转绿）。**复审更正**：原 TC-PR28-8 `background_execute_ends_task_failed_on_skill_exception` 因 `async def _sf()` 触发 coroutine 协议 TypeError → INTERNAL_ERROR 假绿，已在 commit 7 删除并拆为 8a / 8b。
- **本地门禁**：backend `pytest` **155 passed / 2 skipped**（基线 144 + 11 PR-28 净增，与 §七 期望 155 完全吻合，不自行调期望值）· `ruff check app/ tests/` **All checks passed**（PR-28 改动文件全 clean；`ruff check .` 仅 untracked UAT 工具 `_diag_uat.py` / `_seed_resume2_uat.py` 报错，非 PR 范围）· frontend **零改动**（`git diff --stat master -- frontend/` 为空）· §七 第四道门 **e2e 未真做**（复审判定）：TC-PR28-4 仅为单元级单测，非 curl e2e；§4.1 A1-A9 浏览器验收（`'parsed_content' is a required property` 是否被证伪）留待指挥官。

### §9.3.23 无护栏区分"我连的是哪个后端"（OPEN · 建议 Stage 5.1 顺手收）
- **状态**：OPEN · 非阻塞 MVP · 由 2026-07-27 §4.1 验收误诊事故催生（详见 §9.4 陷阱 17）
- **问题**：Windows 下两个 uvicorn 可同时 LISTENING `:8000`，请求被内核随机分流；旧进程执行旧代码 + 旧 `.env` 快照，报出的错误（`Connection error.`）与真实原因毫无关联，把排查引向 LLM 配置。整个链路上**没有任何一处**能回答"我这个请求是被哪个进程、哪个 commit 的代码处理的"。
- **建议修复（两项，均小）**：① **起服 fail-fast** —— `app/main.py` lifespan 启动前主动 `bind` 探测目标端口，已被占用则直接 `sys.exit` 并打印占用 PID，而不是静默共享端口；② **`/health` 增加身份字段** —— 返回 `{"git_sha": <启动时读取的 HEAD>, "started_at": <进程启动时间>, "pid": os.getpid()}`，使"验收前确认后端是新的"变成一条 curl。
- **附带收益**：②让 MVP-VERIFY CHECKLIST §0 可以加一条机器可校验的前置断言（`git_sha` == 本地 `git rev-parse HEAD`），把陷阱 17 的人工纪律变成自动检查。
- **不做的理由（当前）**：属工程健壮性，不影响 MVP 功能演示；等 Stage 5.1 P3 动 `main.py` 时一并做，避免为此单开 PR。
