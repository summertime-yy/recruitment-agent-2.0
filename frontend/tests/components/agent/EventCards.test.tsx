// PR-19 S5-13 · EventCards 单元测试(commit 1 占位骨架 · red via test.fails)。
// 落点:TC-S5-13-3 / TC-S5-13-7 / TC-S5-13-8 / TC-S5-13-10。
// 组件 API 契约(实现见 commit 2/5):
...
describe('EventCards (S5-13)', () => {
  test.fails('TC-S5-13-3 8 类事件 → 7 类 Card 出现在 timeline(system 不进列表)', () => {
    render(
      <ConfigProvider>
        <MessageTimeline
          events={[
    ...
    expect(screen.getByText('search_resumes')).toBeInTheDocument();
  ...
  test.fails('TC-S5-13-7 warning 事件 → WarningCard 显示 message', () => {
    render(
      <ConfigProvider>
  ...
  test.fails('TC-S5-13-8 system 事件 → 顶部条显示 message · 不进 timeline', () => {
    render(
      <ConfigProvider>
  ...
  test.fails('TC-S5-13-10 error 事件 → ErrorCard 显示 message', () => {
    render(
      <ConfigProvider>
