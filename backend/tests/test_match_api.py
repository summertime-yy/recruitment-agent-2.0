...
async def test_post_match_score_returns_201(
    ...
    resp = await client_db.post(
        "/api/v1/match-scores", json={"jd_id": jd.jd_id, "resume_id": resume.resume_id}
    )
...
async def test_post_match_score_404_when_jd_missing(
    ...
    resp = await client_db.post(
        "/api/v1/match-scores", json={"jd_id": "jd_none", "resume_id": resume.resume_id}
    )
...
async def test_post_match_score_404_when_resume_missing(
    ...
    resp = await client_db.post(
        "/api/v1/match-scores", json={"jd_id": jd.jd_id, "resume_id": "res_none"}
    )
    assert resp.status_code == 404
...
async def test_post_match_score_force_recomputes(
    client_db: AsyncClient, db_session: AsyncSession, monkeypatch
) -> None:
    ...
    r2 = await client_db.post(
        "/api/v1/match-scores", json={**body, "force": True}
    )
    second = r2.json()
...
async def test_post_batch_match_returns_task_id(
    client_db: AsyncClient, db_session: AsyncSession, monkeypatch
) -> None:
    _patch_llm(monkeypatch)
    jd = build_jd()
    ...
    resp = await client_db.post(
        "/api/v1/match-scores/batch", json={"jd_id": jd.jd_id, "limit": 2}
    )
    assert resp.status_code == 202
    assert "task_id" in resp.json()


async def test_get_batch_status_transitions(
    client_db: AsyncClient, db_session: AsyncSession, monkeypatch
) -> None:
    _patch_llm(monkeypatch)
    jd = build_jd()
...
async def test_get_score_by_id_returns_row(
    client_db: AsyncClient, db_session: AsyncSession, monkeypatch
) -> None:
    _patch_llm(monkeypatch)
    jd = build_jd()
