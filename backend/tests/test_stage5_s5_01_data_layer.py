...
async def _read_current_head(async_url: str) -> str | None:
    """异步读取共享 dev 库当前 alembic head（用项目既有的 asyncpg 驱动）。"""
...
def _current_head(async_url: str) -> str | None:
    """同步包装：在模块级 sync fixture 内调用（不在事件循环，可安全 asyncio.run）。"""
    return asyncio.run(_read_current_head(async_url))
...
def _migrated_schema() -> None:
    ...
    settings = get_settings()
    async_url = settings.database_url  # 项目使用 asyncpg 驱动

    pre_head = _current_head(async_url)
...
# --- TC-S5-01-1 ---------------------------------------------------------------
async def _inspect_schema(async_url: str):
    """基于项目 asyncpg 引擎同步式 inspect：返回 (表名集合, tasks索引集合, executions索引集合)。"""
    engine = create_async_engine(async_url, poolclass=NullPool)
    try:
        async with engine.connect() as conn:
            def _run(sync_conn):
...
async def test_tc_s5_01_1_migration_creates_tasks_executions(_migrated_schema):
    """升级 head 后 tasks / executions 两表存在，且复合索引已建。"""
    settings = get_settings()
    tables, task_indexes, exec_indexes = await _inspect_schema(settings.database_url)
