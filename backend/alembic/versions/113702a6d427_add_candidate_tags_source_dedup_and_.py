...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('note_type', sa.String(length=20), server_default='NOTE', nullable=False,
    ...
    op.add_column('resumes', sa.Column(
        'duplicate_of_resume_id', sa.String(length=50), nullable=True,
        comment='疑似重复的源简历ID（候选人级去重，硬匹配命中后填入）'))
    op.add_column('resumes', sa.Column(
        'dedup_status', sa.String(length=20), server_default='NONE', nullable=False,
        comment='去重状态：NONE/SUSPECTED/CONFIRMED_DUP/IGNORED'))
    op.create_foreign_key(
        'fk_resumes_duplicate_of_resume_id', 'resumes', 'resumes',
        ['duplicate_of_resume_id'], ['resume_id'], ondelete='SET NULL',
    )
