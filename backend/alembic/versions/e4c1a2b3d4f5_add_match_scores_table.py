...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "resume_updated_at_snapshot", sa.DateTime(), nullable=True, comment="生成时简历updated_at快照，用于陈旧判断"
        ),
        sa.Column("jd_updated_at_snapshot", sa.DateTime(), nullable=True, comment="生成时JD updated_at快照"),
        sa.Column(
            "status", sa.String(length=20), server_default="COMPLETED", nullable=False, comment="COMPLETED/FAILED/STALE"
        ),
    ...
    op.create_index(
        "idx_match_scores_jd_id_overall",
        "match_scores",
        ["jd_id", sa.text("overall_score DESC")],
        unique=False,
    )
